...
class TaskController extends Controller
{
    ...
    public function updateStatus($id, Request $request)
    {
        ...
        // Eğer yeni durum "Onay Bekliyor" ise tarih ekleyelim
        if ($request->status === "Onay Bekliyor") {
            $task->approval_requested_at = now();
        } 
        // Eğer yeni durum "Onaylandı" ise tarih ekleyelim
        elseif ($request->status === "Onaylandı") {
    ...
    {
        // Task’ın durumunu "Arşivlendi" olarak güncelleyin
        $task->update(['status' => 'Arşivlendi']);

        // İşlem sonrası kullanıcıyı bir önceki sayfaya yönlendirip, mesaj gönderebilirsiniz.
        return redirect()->back()->with('success', 'Task arşivlendi.');
    }
    ...
    {
        // Sadece task'ın statusu Arşivlendi ise güncelleme yapalım
        if ($task->status === 'Arşivlendi') {
            $task->update(['status' => 'Onaylandı']);
        }

        // İşlem sonrası kullanıcıyı geri yönlendirip, mesaj ile bilgilendirebilirsiniz.
        return redirect()->back()->with('success', 'Task arşivden çıkarıldı.');
    }
