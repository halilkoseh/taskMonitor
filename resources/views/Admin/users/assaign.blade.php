...
@section('content')
    <div id="mainContent" class="flex-1 p-8 mt-8  ">
        ...
        <form class=" mx-auto bg-white p-8 rounded-lg shadow-lg space-y-6 w-1/2" action="{{ route('admin.tasks.store') }}"
            ...
            <div class="mb-4">
                <label for="project" class="block text-gray-700 font-bold mb-2">Proje - Müdürü</label>
                <select id="project" name="project"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    required>
                    @foreach ($projects as $project)
                        <option value="{{ $project->id }}">
                            {{ $project->name }} - {{ $project->manager->name ?? 'Yönetici Yok' }}
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div id="assignedUsers" class="space-y-4">
                <div class="mb-4">
                    ...
                    <!-- FontAwesome CDN bağlantınızı eklemeyi unutmayın -->
            ...
            <div class="mb-4">
                <label for="attachments" class="block  text-gray-700 font-bold mb-2">
                    Ek Materyaller (.zip önerilir)
                </label>
                <input type="file" id="attachments" name="attachments"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    accept=".jpeg,.jpg,.png,.svg,.gif,.pdf,.zip">
            </div>
    ...
    <div id="taskModal" class="fixed inset-0 flex items-center justify-center z-50 hidden">
        <div class="bg-white p-8 rounded-lg shadow-xl max-w-sm mx-auto text-center relative">
            ...
            <button
                id="closeModal"="bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 text-white font-semibold py-3 px-6 rounded-full shadow-md transition-transform transform hover:scale-105">
