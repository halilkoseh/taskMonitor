...
<div class="content-container mx-auto">
    <div class="flex flex-wrap justify-between items-center mb-8 p-2">
        ...
        <div class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 space-x-0 sm:space-x-8 w-full sm:w-auto mt-4 sm:mt-0">
            ...
            <div class="user-info-container relative pr-8">
    ...
    <div class="container mx-auto p-4">
        ...
        <div class="bg-transparent py-3 px-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 task-board text-gray-600">
                @foreach (['Atandı', 'Devam Ediyor', 'Revize', 'Onay Bekliyor', 'Onaylandı'] as $index => $status) @php $statusCount = $userTasks->pluck('tasks')->flatten()->where('status', $status)->count(); @endphp
                    ...
                    <div class="task-card p-4 mb-4 rounded-lg shadow-md relative bg-white" draggable="true" data-task-id="{{ $task->id }}" ondragstart="handleDragStart(event)" ondragend="handleDragEnd(event)">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="ml-4 space-x-2">
                                ...
                                <a href="{{ route('attachments.download', ['filename' => basename($task->attachments)]) }}" class="text-gray-600 hover:text-blue-500"><i class="fa-solid fa-paperclip mr-2"></i></a>
                            ...
                            @if ($task->status === 'Onaylandı')
                            <div class="mr-2">
                                <!-- Form: Tıklanıldığında task arşivlenecek -->
                                <form action="{{ route('tasks.archive', $task->id) }}" method="POST" onsubmit="return confirm('Bu task arşivlenecek, emin misiniz?')">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="focus:outline-none">
                                        <i class="fa-solid fa-box-open color-blue-300"></i>
                                    </button>
                                </form>
                            </div>
                        @endif
                        ...
                        <div class="task-actions absolute top-2 right-2"></div>
                    </div>
